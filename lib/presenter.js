(function() {
  ...
  Presenter = (function() {
    ...
    Presenter.prototype.play = function(start, end) {
      ...
      file = this.pos.file();
      _mode = this.compiler.mode_id;
      if (file.type() === 'coffeescript') {
        this.compiler.set_mode('coffee');
      } else {
        this.compiler.set_mode('js');
      }
      this.method_missing(file.by_lines(start, end));
      this.compiler.mode_id = _mode;
      return true;
    };
    ...
    Presenter.prototype.method_missing = function(input) {
      ...
      try {
        ...
        console.log("=> ", new SyncHighlight(output || 'undefined').highlight());
      } catch (_error) {
        err = _error;
        this.last_error = err;
